...
plot(grids.a,v_firm,'--','LineWidth',3,'Color',[0,0,0,1])
grid on
xlabel('a')
ylabel('V(a)')
ylim([min([v_implicit', v_firm'])*0.8, max([v_implicit', v_firm'])*1.3])
legend('Endowment Economy (y=1)','Production Economy ','Location','northeast','FontSize',12, 'Interpreter', 'tex'); 
...
plot(grids.a,adot_firm,'--','LineWidth',3,'Color',[0,0,0,1])
grid on
xlabel('a')
ylabel('$\dot{a}$', 'Interpreter', 'latex', 'FontSize',20)
ylim([min([adot_implicit', adot_firm'])*1.3, max([adot_implicit', adot_firm'])*1.3])
legend('Endowment Economy (y=1)','Production Economy ','Location','northeast','FontSize',12, 'Interpreter', 'tex'); 
xlim([num.a_min num.a_max])
...
plot(grids.a,c_firm,'--','LineWidth',3,'Color',[0,0,0,1])
grid on
xlabel('a')
ylabel('c(a)')
ylim([min([c_implicit', c_firm'])*0.8, max([c_implicit', c_firm'])*1.3])
legend('Endowment Economy (y=1)','Production Economy ','Location','northeast','FontSize',12, 'Interpreter', 'tex'); 
xlim([num.a_min num.a_max])
